...
namespace Idp.Swiyu.Passkeys.Sts.Services;
...
public class EmailSender : Microsoft.AspNetCore.Identity.UI.Services.IEmailSender
{
    ...
    public async Task SendEmailAsync(string email, string subject, string htmlMessage)
    {
        ...
        msg.SetFrom(new EmailAddress(_optionsEmailSettings.Value.SenderEmailAddress, "ahead"));
        msg.AddTo(new EmailAddress(email));
        msg.SetSubject(subject);
        //msg.AddContent(MimeType.Text, message);
        msg.AddContent(MimeType.Html, htmlMessage);

        msg.SetReplyTo(new EmailAddress(_optionsEmailSettings.Value.SenderEmailAddress, "ahead"));
